...
#' @description `lotri_corrtext()` and `utri_corrtext()` are
#'   used to display text labels for the strength of bivariate correlations
#'   in the off-diagonal facets of [ggcorrm] plots.
...
#'   (defaults to 2).
#' @param corr_size logical - should the `size` aesthetic be expressed
#'   as a function of correlation strength? `corr_size = TRUE` is a
#'   shorthand for setting `aes(size = abs(..corr..))`. Similar
#'   expressions can be used to access the correlation calculated by
#'   `stat_corrtext` manually in `aes()`. Defaults to
#'   `TRUE`.
#' @param corr_method character string with the correlation method passed
#'   to [cor()][stats]. Can be one of `"pearson"`, `"kendall"` and
...
#' @details `lotri_corrtext()` and `utri_corrtext()` can be used
#'   to display the correlation  between variables in the lower or upper
#'   triangular facets of `ggcorrm` plots, respectively.
#'   Correlations can be calculated for single groups
#'   by using grouping aesthetics such as `aes(color = group)`.
#'
#'  `lotri_corrtext()` and `utri_corrtext()` are wrappers around
#'  [stat_corrtext()] that additionally take
#'  [stat_corrtext()] that additionally take
#'  care of the right specification of aesthetics and allows to easily
#'  adjust size by correlation strength via `corr_size`.
#'
#' @seealso
#'   [stat_corrtext]
#' @name add_corrtext
