...
## Motivation
The inspection of scatterplot matrices and correlations between variables is a central part of exploratory data analysis. 
As collinearity between predictors has the potential to seriously compromise the interpretability of regression results and to blow up the standard errors of regression coefficients, the inspection of the correlations among potential predictor variables is recommended before fitting any regression model with more than one predictor. 
Moreover, in Bayesian data analysis, it often makes sense to inspect posterior correlations between model parameters to identify convergence issues.

There are plenty of reasons why one might want to visually inspect correlation matrices. However, while R provides plenty of functions and packages to display correlations, so far most of them are limited in their flexibility, and none truly leverage the full flexibility of `ggplot2`. 

`corrmorant` is meant to give you this flexibility. It is designed to blend in seamlessly in a data analysis workflow based on the `tidyverse`. It offers a broad variety of functions that allow you to create a large number of different correlation plots, and can be extended easily with new geoms, stats and themes, just as `ggplot2` itself. 
